...
public class IdenticalSubMixedGraphTest
		extends AbstractIdenticalSubMixedGraphTest {
	
	private Predicate<Node> allNodes = Predicates.alwaysTrue();
	
	private Predicate<Edge> allEdges = Predicates.alwaysTrue();
	
	@Before
	public void setUp() {
		original = new ListMixedGraph();
		identicalSubGraph = new SubMixedGraph(original, allNodes, allEdges);
	}
}
